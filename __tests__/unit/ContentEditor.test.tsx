...
describe('ContentEditor Component', () => {
  it('updates the title when input changes', async () => {
    render(
      <DocumentContext.Provider value={mockContext as any}>
        <ContentEditor />
      </DocumentContext.Provider>
    );

    const input = screen.getByPlaceholderText('Note title...') as HTMLInputElement;
    ...
    await waitFor(() => {
        expect(mockUpdateDocument).toHaveBeenCalledWith('1', { title: 'New Test Title' });
    }, { timeout: 1000 });
  });
  ...
  it('updates title immediately on Enter key', () => {
    ...
    expect(mockUpdateDocument).toHaveBeenCalledWith('1', { title: 'New Title Enter' });
  });
});
