import require$$1$3, { app, session, ipcMain, BrowserWindow, shell } from "electron";
...
import path$y from "node:path";
import { fileURLToPath } from "node:url";
import fs$A from "fs/promises";
const is = {
  dev: !app.isPackaged
};
const platform$1 = {
...
if (!fs$y[gracefulQueue]) {
  var queue = commonjsGlobal[gracefulQueue] || [];
  publishQueue(fs$y, queue);
  fs$y.close = function(fs$close) {
    function close(fd, cb) {
      return fs$close.call(fs$y, fd, function(err) {
        if (!err) {
          resetQueue();
        }
  ...
  fs$y.closeSync = function(fs$closeSync) {
    function closeSync(fd) {
      fs$closeSync.apply(fs$y, arguments);
      resetQueue();
    }
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || "")) {
    process.on("exit", function() {
      debug$3(fs$y[gracefulQueue]);
      require$$5.equal(fs$y[gracefulQueue].length, 0);
    });
  }
}
if (!commonjsGlobal[gracefulQueue]) {
  publishQueue(commonjsGlobal, fs$y[gracefulQueue]);
}
var gracefulFs = patch$2(clone(fs$y));
if (process.env.TEST_GRACEFUL_FS_GLOBAL_PATCH && !fs$y.__patched) {
  gracefulFs = patch$2(fs$y);
  fs$y.__patched = true;
}
...
function enqueue(elem) {
  ...
  fs$y[gracefulQueue].push(elem);
  retry$2();
}
var retryTimer;
function resetQueue() {
  var now = Date.now();
  for (var i = 0; i < fs$y[gracefulQueue].length; ++i) {
    if (fs$y[gracefulQueue][i].length > 2) {
      fs$y[gracefulQueue][i][3] = now;
      fs$y[gracefulQueue][i][4] = now;
    }
  }
  retry$2();
}
function retry$2() {
  ...
  retryTimer = void 0;
  if (fs$y[gracefulQueue].length === 0)
    return;
  var elem = fs$y[gracefulQueue].shift();
  var fn = elem[0];
  var args = elem[1];
  var err = elem[2];
...
var makeDir$3 = {};
var utils$2 = {};
const path$x = require$$1$1;
utils$2.checkPath = function checkPath(pth) {
  if (process.platform === "win32") {
    const pathHasInvalidWinCharacters = /[<>:"|?*]/.test(pth.replace(path$x.parse(pth).root, ""));
...
function pathExists$d(path2) {
  return fs$w.access(path2).then(() => true).catch(() => false);
}
var pathExists_1$1 = {
  pathExists: u$o(pathExists$d),
  pathExistsSync: fs$w.existsSync
};
const fs$v = gracefulFs;
function utimesMillis$3(path2, atime, mtime, callback) {
  fs$v.open(path2, "r+", (err, fd) => {
    if (err) return callback(err);
    fs$v.futimes(fd, atime, mtime, (futimesErr) => {
      fs$v.close(fd, (closeErr) => {
...
function utimesMillisSync$3(path2, atime, mtime) {
  const fd = fs$v.openSync(path2, "r+");
  fs$v.futimesSync(fd, atime, mtime);
  return fs$v.closeSync(fd);
}
var utimes$1 = {
...
const fs$u = fs$z;
const path$w = require$$1$1;
const util$1 = require$$4;
function getStats$4(src2, dest, opts) {
  const statFunc = opts.dereference ? (file2) => fs$u.stat(file2, { bigint: true }) : (file2) => fs$u.lstat(file2, { bigint: true });
  return Promise.all([
    statFunc(src2),
...
function getStatsSync$1(src2, dest, opts) {
  let destStat;
  const statFunc = opts.dereference ? (file2) => fs$u.statSync(file2, { bigint: true }) : (file2) => fs$u.lstatSync(file2, { bigint: true });
...
function checkPaths$1(src2, dest, funcName, opts, cb) {
  util$1.callbackify(getStats$4)(src2, dest, opts, (err, stats) => {
    if (err) return cb(err);
    const { srcStat, destStat } = stats;
    if (destStat) {
      if (areIdentical$5(srcStat, destStat)) {
        const srcBaseName = path$w.basename(src2);
        const destBaseName = path$w.basename(dest);
...
function checkParentPaths$1(src2, srcStat, dest, funcName, cb) {
  const srcParent = path$w.resolve(path$w.dirname(src2));
  const destParent = path$w.resolve(path$w.dirname(dest));
  if (destParent === srcParent || destParent === path$w.parse(destParent).root) return cb();
  fs$u.stat(destParent, { bigint: true }, (err, destStat) => {
    if (err) {
...
function checkParentPathsSync$1(src2, srcStat, dest, funcName) {
  const srcParent = path$w.resolve(path$w.dirname(src2));
  const destParent = path$w.resolve(path$w.dirname(dest));
  if (destParent === srcParent || destParent === path$w.parse(destParent).root) return;
  let destStat;
  try {
    destStat = fs$u.statSync(destParent, { bigint: true });
  } catch (err) {
...
function isSrcSubdir$1(src2, dest) {
  const srcArr = path$w.resolve(src2).split(path$w.sep).filter((i) => i);
  const destArr = path$w.resolve(dest).split(path$w.sep).filter((i) => i);
...
function checkParentDir(destStat, src2, dest, opts, cb) {
  const destParent = path$v.dirname(dest);
...
function getStats$3(destStat, src2, dest, opts, cb) {
  const stat2 = opts.dereference ? fs$t.stat : fs$t.lstat;
...
function setDestMode$2(dest, srcMode, cb) {
  return fs$t.chmod(dest, srcMode, cb);
}
function setDestTimestamps$2(src2, dest, cb) {
  fs$t.stat(src2, (err, updatedSrcStat) => {
    if (err) return cb(err);
...
function copyDirItems(items, src2, dest, opts, cb) {
  ...
  if (!item) return cb();
  return copyDirItem$2(items, item, src2, dest, opts, cb);
}
function copyDirItem$2(items, item, src2, dest, opts, cb) {
  const srcItem = path$v.join(src2, item);
  const destItem = path$v.join(dest, item);
...
function onLink$3(destStat, src2, dest, opts, cb) {
  fs$t.readlink(src2, (err, resolvedSrc) => {
    if (err) return cb(err);
    if (opts.dereference) {
      resolvedSrc = path$v.resolve(process.cwd(), resolvedSrc);
    }
    if (!destStat) {
      return fs$t.symlink(resolvedSrc, dest, cb);
    } else {
      fs$t.readlink(dest, (err2, resolvedDest) => {
        if (err2) {
          if (err2.code === "EINVAL" || err2.code === "UNKNOWN") return fs$t.symlink(resolvedSrc, dest, cb);
          return cb(err2);
        }
        if (opts.dereference) {
          resolvedDest = path$v.resolve(process.cwd(), resolvedDest);
        }
...
function handleFilterAndCopy(destStat, src2, dest, opts) {
  ...
  const destParent = path$u.dirname(dest);
  if (!fs$s.existsSync(destParent)) mkdirsSync$3(destParent);
...
function getStats$2(destStat, src2, dest, opts) {
  const statSync = opts.dereference ? fs$s.statSync : fs$s.lstatSync;
...
function setDestMode$1(dest, srcMode) {
  return fs$s.chmodSync(dest, srcMode);
}
function setDestTimestamps$1(src2, dest) {
  const updatedSrcStat = fs$s.statSync(src2);
...
function copyDir$1(src2, dest, opts) {
  fs$s.readdirSync(src2).forEach((item) => copyDirItem$1(item, src2, dest, opts));
}
function copyDirItem$1(item, src2, dest, opts) {
  const srcItem = path$u.join(src2, item);
  const destItem = path$u.join(dest, item);
  const { destStat } = stat$7.checkPathsSync(srcItem, destItem, "copy", opts);
  return startCopy(destStat, srcItem, destItem, opts);
}
function onLink$2(destStat, src2, dest, opts) {
  let resolvedSrc = fs$s.readlinkSync(src2);
  if (opts.dereference) {
    resolvedSrc = path$u.resolve(process.cwd(), resolvedSrc);
  }
  if (!destStat) {
    return fs$s.symlinkSync(resolvedSrc, dest);
  } else {
    let resolvedDest;
    try {
      resolvedDest = fs$s.readlinkSync(dest);
    } catch (err) {
      if (err.code === "EINVAL" || err.code === "UNKNOWN") return fs$s.symlinkSync(resolvedSrc, dest);
      throw err;
    }
    if (opts.dereference) {
      resolvedDest = path$u.resolve(process.cwd(), resolvedDest);
    }
    if (stat$7.isSrcSubdir(resolvedSrc, resolvedDest)) {
      throw new Error(`Cannot copy '${resolvedSrc}' to a subdirectory of itself, '${resolvedDest}'.`);
    }
    if (fs$s.statSync(dest).isDirectory() && stat$7.isSrcSubdir(resolvedDest, resolvedSrc)) {
...
function rmkids(p, options, cb) {
  ...
  options.readdir(p, (er, files) => {
    ...
    files.forEach((f) => {
      rimraf$1(path$t.join(p, f), options, (er2) => {
        if (errState) {
          return;
        }
...
function rmkidsSync(p, options) {
  assert(p);
  assert(options);
  options.readdirSync(p).forEach((f) => rimrafSync(path$t.join(p, f), options));
  if (isWindows) {
...
function remove$5(path2, callback) {
  if (fs$q.rm) return fs$q.rm(path2, { recursive: true, force: true }, callback);
  rimraf(path2, callback);
}
function removeSync$3(path2) {
  if (fs$q.rmSync) return fs$q.rmSync(path2, { recursive: true, force: true });
  rimraf.sync(path2);
}
var remove_1$1 = {
  remove: u$m(remove$5),
...
const emptyDir$1 = u$l(async function emptyDir(dir) {
  let items;
  try {
    items = await fs$p.readdir(dir);
  } catch {
    return mkdir$7.mkdirs(dir);
  }
  return Promise.all(items.map((item) => remove$4.remove(path$s.join(dir, item))));
});
function emptyDirSync$1(dir) {
  let items;
  try {
    items = fs$p.readdirSync(dir);
  } catch {
    return mkdir$7.mkdirsSync(dir);
  }
  items.forEach((item) => {
    item = path$s.join(dir, item);
...
function createFile$3(file2, callback) {
  function makeFile() {
    fs$o.writeFile(file2, "", (err) => {
      if (err) return callback(err);
      callback();
    });
  }
  fs$o.stat(file2, (err, stats) => {
    if (!err && stats.isFile()) return callback();
    const dir = path$r.dirname(file2);
    fs$o.stat(dir, (err2, stats2) => {
      if (err2) {
        if (err2.code === "ENOENT") {
...
function createFileSync$3(file2) {
  let stats;
  try {
    stats = fs$o.statSync(file2);
  } catch {
  }
  if (stats && stats.isFile()) return;
  const dir = path$r.dirname(file2);
  try {
    if (!fs$o.statSync(dir).isDirectory()) {
      fs$o.readdirSync(dir);
    }
  } catch (err) {
    if (err && err.code === "ENOENT") mkdir$6.mkdirsSync(dir);
    else throw err;
  }
  fs$o.writeFileSync(file2, "");
}
var file$1 = {
...
function createLink$3(srcpath, dstpath, callback) {
  function makeLink(srcpath2, dstpath2) {
    fs$n.link(srcpath2, dstpath2, (err) => {
      if (err) return callback(err);
      callback(null);
    });
  }
  fs$n.lstat(dstpath, (_, dstStat) => {
    fs$n.lstat(srcpath, (err, srcStat) => {
      if (err) {
        err.message = err.message.replace("lstat", "ensureLink");
        return callback(err);
      }
      if (dstStat && areIdentical$4(srcStat, dstStat)) return callback(null);
      const dir = path$q.dirname(dstpath);
...
function createLinkSync$3(srcpath, dstpath) {
  let dstStat;
  try {
    dstStat = fs$n.lstatSync(dstpath);
  } catch {
  }
  try {
    const srcStat = fs$n.lstatSync(srcpath);
  ...
  const dir = path$q.dirname(dstpath);
  const dirExists = fs$n.existsSync(dir);
  if (dirExists) return fs$n.linkSync(srcpath, dstpath);
  mkdir$5.mkdirsSync(dir);
  return fs$n.linkSync(srcpath, dstpath);
}
var link$1 = {
...
function symlinkPaths$3(srcpath, dstpath, callback) {
  if (path$p.isAbsolute(srcpath)) {
    return fs$m.lstat(srcpath, (err) => {
      if (err) {
    ...
    const dstdir = path$p.dirname(dstpath);
    const relativeToDst = path$p.join(dstdir, srcpath);
    return pathExists$a(relativeToDst, (err, exists) => {
      ...
      if (exists) {
        ...
        return fs$m.lstat(srcpath, (err2) => {
          if (err2) {
          ...
          return callback(null, {
            toCwd: srcpath,
            toDst: path$p.relative(dstdir, srcpath)
          });
        });
      }
    });
...
function symlinkPathsSync$3(srcpath, dstpath) {
  let exists;
  if (path$p.isAbsolute(srcpath)) {
    exists = fs$m.existsSync(srcpath);
    ...
    const dstdir = path$p.dirname(dstpath);
    const relativeToDst = path$p.join(dstdir, srcpath);
    exists = fs$m.existsSync(relativeToDst);
    if (exists) {
      return {
        toCwd: relativeToDst,
        toDst: srcpath
  ...
  symlinkPaths$2(srcpath, dstpath, (err, relative) => {
    ...
    symlinkType$2(relative.toCwd, type2, (err2, type3) => {
      ...
      pathExists$9(dir, (err3, dirExists) => {
        if (err3) return callback(err3);
        if (dirExists) return fs$k.symlink(srcpath, dstpath, type3, callback);
        mkdirs$3(dir, (err4) => {
          if (err4) return callback(err4);
          fs$k.symlink(srcpath, dstpath, type3, callback);
        });
      });
    });
  });
...
function createSymlinkSync$3(srcpath, dstpath, type2) {
  let stats;
  try {
    stats = fs$k.lstatSync(dstpath);
  } catch {
  }
  if (stats && stats.isSymbolicLink()) {
    const srcStat = fs$k.statSync(srcpath);
    const dstStat = fs$k.statSync(dstpath);
  ...
  const dir = path$o.dirname(dstpath);
  const exists = fs$k.existsSync(dir);
  if (exists) return fs$k.symlinkSync(srcpath, dstpath, type2);
  mkdirsSync$2(dir);
  return fs$k.symlinkSync(srcpath, dstpath, type2);
}
var symlink$1 = {
...
function outputFile$3(file2, data, encoding, callback) {
  ...
  const dir = path$n.dirname(file2);
  pathExists$8(dir, (err, itDoes) => {
    if (err) return callback(err);
    if (itDoes) return fs$j.writeFile(file2, data, encoding, callback);
    mkdir$4.mkdirs(dir, (err2) => {
      if (err2) return callback(err2);
      fs$j.writeFile(file2, data, encoding, callback);
    });
  });
}
function outputFileSync$3(file2, ...args) {
  ...
  if (fs$j.existsSync(dir)) {
    return fs$j.writeFileSync(file2, ...args);
  }
  mkdir$4.mkdirsSync(dir);
  fs$j.writeFileSync(file2, ...args);
}
var outputFile_1$1 = {
...
function move$3(src2, dest, opts, cb) {
  ...
  stat$6.checkPaths(src2, dest, "move", opts, (err, stats) => {
    ...
    stat$6.checkParentPaths(src2, srcStat, dest, "move", (err2) => {
      ...
      mkdirp$1(path$m.dirname(dest), (err3) => {
...
function isParentRoot$2(dest) {
  const parent = path$m.dirname(dest);
  const parsedPath = path$m.parse(parent);
...
function moveSync$1(src2, dest, opts) {
  ...
  if (!isParentRoot$1(dest)) mkdirpSync$1(path$l.dirname(dest));
  return doRename$2(src2, dest, overwrite, isChangingCase);
}
function isParentRoot$1(dest) {
  const parent = path$l.dirname(dest);
  const parsedPath = path$l.parse(parent);
  return parsedPath.root === parent;
}
function doRename$2(src2, dest, overwrite, isChangingCase) {
  ...
  if (fs$h.existsSync(dest)) throw new Error("dest already exists.");
  return rename$1(src2, dest, overwrite);
}
function rename$1(src2, dest, overwrite) {
  try {
    fs$h.renameSync(src2, dest);
  } catch (err) {
...
class DownloadedUpdateHelper {
  ...
  get cacheDirForPendingUpdate() {
    return path$k.join(this.cacheDir, "pending");
  }
  ...
  async getValidCachedUpdateFile(fileInfo, logger) {
    ...
    const updateFile = path$k.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
  ...
  getUpdateInfoFile() {
    return path$k.join(this.cacheDirForPendingUpdate, "update-info.json");
  }
}
...
async function createTempUpdateFile(name, cacheDir, log) {
  let nameCounter = 0;
  let result = path$k.join(cacheDir, name);
  for (let i = 0; i < 3; i++) {
    try {
      ...
      result = path$k.join(cacheDir, `${nameCounter++}-${name}`);
    }
  }
  return result;
}
...
function getAppCacheDir() {
  ...
  let result;
  if (process.platform === "win32") {
    result = process.env["LOCALAPPDATA"] || path$j.join(homedir2, "AppData", "Local");
  } else if (process.platform === "darwin") {
    result = path$j.join(homedir2, "Library", "Caches");
  } else {
    result = process.env["XDG_CACHE_HOME"] || path$j.join(homedir2, ".cache");
  }
  return result;
}
...
class ElectronAppAdapter {
  ...
  get appUpdateConfigPath() {
    return this.isPackaged ? path$i.join(process.resourcesPath, "app-update.yml") : path$i.join(this.app.getAppPath(), "dev-app-update.yml");
  }
  get userDataPath() {
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
  ...
  resolveFiles(updateInfo) {
    return (0, Provider_1$6.getFileList)(updateInfo).map((it) => {
      const name = path$h.posix.basename(it.url).replace(/ /g, "-");
...
class AppUpdater extends events_1.EventEmitter {
  ...
  async getOrCreateStagingUserId() {
    const file2 = path$g.join(this.app.userDataPath, ".updaterId");
    try {
  ...
  async getOrCreateDownloadHelper() {
    ...
    if (result == null) {
      ...
      const cacheDir = path$g.join(this.app.baseCachePath, dirName || this.app.name);
      if (logger.debug != null) {
  ...
  async executeDownload(taskOptions) {
    ...
    function getCacheUpdateFileName() {
      ...
      if (urlPath.toLowerCase().endsWith(`.${taskOptions.fileExtension.toLowerCase()}`)) {
        return path$g.basename(urlPath);
      } else {
    ...
    let updateFile = path$g.join(cacheDir, updateFileName);
    const packageFile = packageInfo == null ? null : path$g.join(cacheDir, `package-${version}${path$g.extname(packageInfo.path) || ".7z"}`);
    const done = async (isSaveCache) => {
      ...
      const currentBlockMapFile = path$g.join(cacheDir, "current.blockmap");
      if (await (0, fs_extra_1$4.pathExists)(currentBlockMapFile)) {
        await (0, fs_extra_1$4.copyFile)(currentBlockMapFile, path$g.join(downloadedUpdateHelper.cacheDir, "current.blockmap"));
      }
  ...
  async differentialDownloadInstaller(fileInfo, downloadUpdateOptions, installerPath, provider, oldInstallerFileName) {
    try {
      ...
      const downloadOptions = {
        newUrl: fileInfo.url,
        oldFile: path$g.join(this.downloadedUpdateHelper.cacheDir, oldInstallerFileName),
        logger: this._logger,
      ...
      const saveBlockMapToCacheDir = async (blockMapData, cacheDir) => {
        const blockMapFile = path$g.join(cacheDir, "current.blockmap");
...
class AppImageUpdater extends BaseUpdater_1$2.BaseUpdater {
  ...
  doInstall(options) {
    ...
    let destination;
    const existingBaseName = path$f.basename(appImageFile);
    const installerPath = this.installerPath;
    if (installerPath == null) {
      this.dispatchError(new Error("No update filepath provided, can't quit and install"));
      return false;
    }
    if (path$f.basename(installerPath) === existingBaseName || !/\d+\.\d+\.\d+/.test(existingBaseName)) {
      destination = appImageFile;
    } else {
      destination = path$f.join(path$f.dirname(appImageFile), path$f.basename(installerPath));
    }
  ...
  async doDownloadUpdate(downloadUpdateOptions) {
    ...
    return this.executeDownload({
      ...
      task: async (destinationFile, downloadOptions) => {
        const cachedUpdateFilePath = path$e.join(this.downloadedUpdateHelper.cacheDir, CURRENT_MAC_APP_ZIP_FILE_NAME);
...
class NsisUpdater extends BaseUpdater_1.BaseUpdater {
  ...
  doInstall(options) {
    ...
    const callUsingElevation = () => {
      this.spawnLog(path$c.join(process.resourcesPath, "elevate.exe"), [installerPath].concat(args)).catch((e) => this.dispatchError(e));
    };
  ...
  async differentialDownloadWebPackage(downloadUpdateOptions, packageInfo, packagePath, provider) {
    ...
    try {
      const downloadOptions = {
        ...
        oldFile: path$c.join(this.downloadedUpdateHelper.cacheDir, builder_util_runtime_1.CURRENT_APP_PACKAGE_FILE_NAME),
        logger: this._logger,
        newFile: packagePath,
...
utils.checkPath = function checkPath2(pth) {
  if (process.platform === "win32") {
...
const fs$f = fs$g;
const { checkPath: checkPath3 } = utils;
...
async function onDir$1(srcStat, destStat, src2, dest, opts) {
  ...
  await asyncIteratorConcurrentProcess(await fs$b.opendir(src2), async (item) => {
...
function copyDir(src2, dest, opts) {
  const dir = fs$a.opendirSync(src2);
  try {
    let dirent;
...
async function createLink$1(srcpath, dstpath) {
  let dstStat;
  try {
    dstStat = await fs$6.lstat(dstpath, { bigint: true });
  } catch {
  }
  let srcStat;
  try {
    srcStat = await fs$6.lstat(srcpath, { bigint: true });
  } catch (err) {
    err.message = err.message.replace("lstat", "ensureLink");
    throw err;
  }
...
function createLinkSync$1(srcpath, dstpath) {
  let dstStat;
  try {
    dstStat = fs$6.lstatSync(dstpath, { bigint: true });
  } catch {
  }
  try {
    const srcStat = fs$6.lstatSync(srcpath, { bigint: true });
    if (dstStat && areIdentical$1(srcStat, dstStat)) return;
  } catch (err) {
    err.message = err.message.replace("lstat", "ensureLink");
    throw err;
...
async function createSymlink$1(srcpath, dstpath, type2) {
  ...
  if (stats && stats.isSymbolicLink()) {
    ...
    if (path$3.isAbsolute(srcpath)) {
      srcStat = await fs$3.stat(srcpath, { bigint: true });
    } else {
      const dstdir = path$3.dirname(dstpath);
      const relativeToDst = path$3.join(dstdir, srcpath);
      try {
        srcStat = await fs$3.stat(relativeToDst, { bigint: true });
      } catch {
        srcStat = await fs$3.stat(srcpath, { bigint: true });
      }
    }
    const dstStat = await fs$3.stat(dstpath, { bigint: true });
...
function createSymlinkSync$1(srcpath, dstpath, type2) {
  ...
  if (stats && stats.isSymbolicLink()) {
    let srcStat;
    if (path$3.isAbsolute(srcpath)) {
      srcStat = fs$3.statSync(srcpath, { bigint: true });
    } else {
      const dstdir = path$3.dirname(dstpath);
      const relativeToDst = path$3.join(dstdir, srcpath);
      try {
        srcStat = fs$3.statSync(relativeToDst, { bigint: true });
      } catch {
        srcStat = fs$3.statSync(srcpath, { bigint: true });
      }
    }
    const dstStat = fs$3.statSync(dstpath, { bigint: true });
    if (areIdentical(srcStat, dstStat)) return;
  }
...
const welcomeData = [
  {
    id: "89e148b2-9c9d-49b8-acd0-ec8a5a382d77",
    type: "paragraph",
    props: {
      textColor: "default",
  ...
  {
    id: "b4bc2f94-47a6-47a1-b3cf-47b56b5ea9c5",
    type: "paragraph",
    props: {
      textColor: "default",
      backgroundColor: "default",
  ...
  {
    id: "0b9881f9-09b1-4d3c-9e1d-3d511b6377af",
    type: "bulletListItem",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "📄 Write and save new documents instantly",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "ba6a4bf8-c18b-4f50-aacb-c8f75c3cc884",
    type: "bulletListItem",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "📖 Read and review your existing work anytime",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "74a1f615-df0a-419d-b1e5-a3a74224874f",
    type: "bulletListItem",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "🗑 Delete outdated or unnecessary files with a single click",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "a497638b-f61d-497b-94ee-3834d918a3bf",
    type: "paragraph",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "Whether you’re drafting ideas, maintaining project notes, or storing important information, Cortex Docs keeps everything organized and accessible.",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "c3e795ae-6a5e-418d-a4ae-5f7b9ab30688",
    type: "paragraph",
    props: {
      textColor: "default",
      backgroundColor: "default",
  ...
  {
    id: "5f1be87a-64a9-4047-b19c-6454a9c73f31",
  ...
  {
    id: "5c658409-1b95-4796-b98d-411a1d20c382",
    type: "numberedListItem",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "Add a title and your content",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "c6bc5746-6334-4dc8-92a5-97b57b805459",
    type: "numberedListItem",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "Save — and it’s ready to read anytime",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "b5f1815c-1b49-4c5b-8429-278066430e41",
    type: "paragraph",
    props: {
      textColor: "default",
      backgroundColor: "default",
    ...
    content: [
      {
        type: "text",
        text: "Your documents, your control — all in one place.",
        styles: {}
      }
    ],
    children: []
  ...
  {
    id: "b0d45a93-2bf6-4d0a-a5ee-3c96724ec7ec",
    type: "paragraph",
    props: {
      textColor: "default",
      backgroundColor: "default",
...
const loadDocs = async () => {
  ...
  if (checkConfig) {
    ...
    const config = await lib.readFile(configPath, "utf-8");
...
const isConfigAvailable = async () => {
  ...
  const configPath = `${rootDir}/config.json`;
  try {
    const configExists = await fs$A.access(configPath).then(() => true).catch(() => false);
    return configExists;
  } catch (error2) {
    return false;
  }
...
const createConfigAndWelcomeFile = async () => {
  ...
  try {
    await lib.writeFile(welcomePath, JSON.stringify(welcomeData, null, 2));
    console.log("Successfully created welcome.json file");
  } catch (error2) {
    console.error("Error creating welcome.json:", error2);
  }
...
const createDoc = async (args) => {
  ...
  const configPath = `${rootDir}/config.json`;
  const existingConfig = await lib.readFile(configPath, "utf-8");
  const configJson = JSON.parse(existingConfig);
  configJson.push(newDoc);
  await lib.writeFile(configPath, JSON.stringify(configJson, null, 2));
...
const deleteDoc = async (args) => {
  ...
  try {
    ...
    await lib.writeFile(configPath, JSON.stringify(updatedConfig, null, 2));
    for (const docId of documentsToDelete) {
      const docFilePath = `${rootDir}/${docId}.json`;
      try {
        await fs$A.unlink(docFilePath);
    ...
    console.error("Error deleting document:", error2);
    return false;
  }
};
...
const updateDoc = async (args) => {
  ...
  try {
    ...
    console.error("Error updating document:", error2);
    return false;
  }
};
const getFileContent = async (args) => {
  ...
  const fileContent = await lib.readFile(filePath, "utf-8");
...
const saveFileContent = async (args) => {
  ...
  const filePath = `${rootDir}/${docId}.json`;
  try {
    await lib.writeFile(filePath, JSON.stringify(content, null, 2));
    return { success: true };
  } catch (error2) {
    console.error("Error saving file content:", error2);
...
const searchDocuments = async (args) => {
  ...
  try {
    ...
    for (const doc of configJson) {
      ...
      if (doc.contentId) {
        try {
          ...
          const content = await lib.readFile(contentPath, "utf-8");
    ...
    console.error("Error searching documents:", error2);
    return [];
  }
};
const __dirname$1 = path$y.dirname(fileURLToPath(import.meta.url));
process.env.APP_ROOT = path$y.join(__dirname$1, "..");
const VITE_DEV_SERVER_URL = process.env["VITE_DEV_SERVER_URL"];
const MAIN_DIST = path$y.join(process.env.APP_ROOT, "dist-electron");
const RENDERER_DIST = path$y.join(process.env.APP_ROOT, "dist");
const getIconPath = () => {
  const platform2 = process.platform;
  const basePath = process.env.APP_ROOT;
  switch (platform2) {
    case "win32":
      return path$y.join(basePath, "public", "icons", "win", "icon.ico");
    case "darwin":
      return path$y.join(basePath, "public", "icons", "mac", "icon.icns");
    case "linux":
    default:
      return path$y.join(basePath, "public", "icons", "png", "256x256.png");
  }
};
const iconPath = getIconPath();
process.env.VITE_PUBLIC = VITE_DEV_SERVER_URL ? path$y.join(process.env.APP_ROOT, "public") : RENDERER_DIST;
let win;
function createWindow() {
  win = new BrowserWindow({
    ...
    // Required for frameless macOS window
    trafficLightPosition: { x: 15, y: 10 },
    transparent: true,
    icon: iconPath,
    webPreferences: {
      preload: path$y.join(__dirname$1, "preload.mjs"),
      // Important for secure IPC
      sandbox: true,
  ...
  main$1.autoUpdater.on("checking-for-update", () => {
    win == null ? void 0 : win.webContents.send("update-message", "Checking for update...");
  });
  main$1.autoUpdater.on("update-available", (info) => {
    win == null ? void 0 : win.webContents.send("update-message", "Update available.", info);
  });
  main$1.autoUpdater.on("update-not-available", (info) => {
    win == null ? void 0 : win.webContents.send("update-message", "Update not available.", info);
  });
  main$1.autoUpdater.on("error", (err) => {
    win == null ? void 0 : win.webContents.send("update-message", `Error in auto-updater. ${err}`);
  });
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(path$y.join(RENDERER_DIST, "index.html"));
  }
}
