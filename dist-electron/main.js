import require$$1$4, { app, session, ipcMain, BrowserWindow, shell } from "electron";
...
const isConfigAvailable = async () => {
  ...
  const configPath = `${rootDir}/config.json`;
  try {
    const configExists = await fs$j.access(configPath).then(() => true).catch(() => false);
    return configExists;
  } catch (error2) {
    return false;
  }
...
const getIconPath = () => {
  ...
  switch (platform2) {
    case "win32":
      return path$m.join(basePath, "public", "icons", "win", "icon.ico");
    case "darwin":
      return path$m.join(basePath, "public", "icons", "mac", "icon.icns");
    case "linux":
    default:
      return path$m.join(basePath, "public", "icons", "png", "256x256.png");
  }
};
